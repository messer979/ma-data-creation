...
from scripts.data_transfer_sync import run_transfer_sync
...
if 'form_items_only' not in st.session_state:
    st.session_state.form_items_only = False
if 'form_inventory_only' not in st.session_state:
    st.session_state.form_inventory_only = False
...
st.set_page_config(page_title="Data Import", layout="wide", page_icon="📥")
...
with tab1:
    ...
    if st.session_state.get('form_items_only', False) and st.session_state.get('form_inventory_only', False):
    ...
    with st.form("inventory_config", enter_to_submit=False):
        ...
        with col5:
            ...
            items_only = st.checkbox(
                "Items only",
                key="form_items_only", 
                value=st.session_state.form_items_only,
                help="Transfer only items, skip inventory adjustments"
            )
            inventory_only = st.checkbox(
                "Inventory only", 
                key="form_inventory_only",
                value=st.session_state.form_inventory_only,
                help="Transfer only inventory adjustments, skip items sync"
            )        
        ...
        conflicting_options = items_only and inventory_only
        if conflicting_options:
            st.error("❌ Cannot select both 'Items only' and 'Inventory only' options!")
    ...
    if st.session_state.get('submitted', False):
        ...
        if len(missing_fields) > 0:
            ...
            config = {
                ...
                'items_only': st.session_state.form_items_only,
                'inventory_only': st.session_state.form_inventory_only
            }
